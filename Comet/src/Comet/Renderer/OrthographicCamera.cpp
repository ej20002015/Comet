...
namespace Comet
{
	...
	void OrthographicCamera::onEvent(Event& e)
	{
		...
		dispatcher.dispatch<MouseScrolledEvent>(CMT_BIND_METHOD(OrthographicCamera::onMouseScrolledEvent));
		dispatcher.dispatch<WindowResizedEvent>(CMT_BIND_METHOD(OrthographicCamera::onWindowResizedEvent));
	}
