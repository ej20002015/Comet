...
std::array<uint32_t, ModelFactory::MESH_TYPE_END> ModelFactory::s_meshCreationCount = { 0 };
...
Reference<Model> ModelFactory::create(MeshType meshType, const Reference<Material>& material, uint32_t LOD)
{
	...
	case ModelFactory::MeshType::SPHERE:
		...
		break;
	default:
		CMT_COMET_ASSERT_MESSAGE(false, "Unkown MeshType - cannot create a model of that type");
		return nullptr;
		break;
	}
}

std::string ModelFactory::getModelIdentifier(MeshType meshType)
{
	return MESH_TYPE_STRS[static_cast<uint32_t>(meshType)] + "_"s + std::to_string(s_meshCreationCount[static_cast<uint32_t>(meshType)]++);
}
...
Reference<Model> ModelFactory::createCube(const Reference<Material>& material)
{
	...
	return createReference<Model>(getModelIdentifier(MeshType::CUBE), cubeVertices, cubeTriangleIndices, material);
}
...
Reference<Model> ModelFactory::createSphere(const Reference<Material>& material, uint32_t LOD)
{
	...
	return createReference<Model>(getModelIdentifier(MeshType::SPHERE), vertices, triangleIndices, material);
}
