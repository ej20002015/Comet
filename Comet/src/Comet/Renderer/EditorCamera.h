...
public:
	...
	void setNearPlane(const float nearPlane) { m_nearPlane = nearPlane; recalculateProjectionMatrix(); }

	float getFarPlane() const { return m_farPlane; }
	void setFarPlane(const float farPlane) { m_farPlane = farPlane; recalculateProjectionMatrix(); }
...
public:
	...
	static constexpr float ZOOM_SPEED = 0.005f;
	static constexpr float PAN_SPEED = 0.2f;
	static constexpr float SHIFT_SPEED_FACTOR = 0.25f;
...
private:
	...
	template<typename T>
	T getShiftModifiedSpeed(const T speed) const;
