...
namespace Comet
{
	...
	SpirvShaderInformation SpirvTools::compileAndReflect(const std::string& shaderSource, const std::string& sourcePath, const std::string& name, const ShaderType type, const ShaderEnvironment shaderEnvironment, const bool optimise)
    {
		...
		compileOrRetrieveBinary(shaderInfo, shaderSource, sourcePath, name, type, optimise);
		reflectOpenGL(shaderInfo);
		
		return shaderInfo;
    }

	void SpirvTools::compileOrRetrieveBinary(SpirvShaderInformation& shaderInfo, const std::string& shaderSource, const std::string& sourcePath, const std::string& name, const ShaderType type, const bool optimise)
	{
		//See if spirv binaries have already been generated for this shader, and if so retrieve them

		std::filesystem::path path = sourcePath;
		path = path.parent_path() / "cached" / (name + ".SPIRVBinary" + getShaderTypeExtension(type));
		...
		if (input)
		{
			...
			shaderInfo.binary = std::vector<uint32_t>(size / sizeof(uint32_t));
			input.read((char*)shaderInfo.binary.data(), size);
			input.close();
		}
		else
		{
			...
			shaderInfo.binary = std::vector<uint32_t>(result.cbegin(), result.cend());
			...
			output.write((char*)shaderInfo.binary.data(), shaderInfo.binary.size() * sizeof(uint32_t));
			output.close();
		}
	}

	void SpirvTools::reflectOpenGL(SpirvShaderInformation& shaderInfo)
	{
		//Set up reflection
		...
		for (auto& resource : resources.uniform_buffers)
		{
			...
			UniformBuffer& uniformBuffer = shaderInfo.uniformBuffers.back();
			...
			for (uint32_t i = 0; i < elementCount; ++i)
			{
				...
				UniformContainerElement element(uniformName, shaderUniformType, uniformSize, uniformOffset);
	...
	void SpirvTools::reflectVulkan(SpirvShaderInformation& shaderInfo)
	{
		CMT_COMET_ASSERT_MESSAGE(false, "Vulkan currently not supported");
	}
